...
namespace FreeCourse.Services.Discount.Services
{
    public class DiscountService : IDiscountService
    {
        ...
        {
            var status = await dbConnection.ExecuteAsync("update discount set userid=@UserId, code=@Code, rate=@Rate where id=@Id",new {Id = discount.Id,UserId = discount.UserId, Code = discount.Code, Rate = discount.Rate});//discount yazmak yerine bu şekilde tek tek de veriler yollanabilir.
            if (status>0)
            {
                return Response<NoContent>.Success(204);
            }
